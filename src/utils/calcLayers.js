...
export default function calcLayers(props) {
  ...
  function generateStreamSizeScale() {
    ...
    return d3
      .scaleLinear()
      .domain([-maxPeriodFreq, maxPeriodFreq])
      .range([0, screenDimensions[1]])
      .nice();
  }
  ...
  function generateStackedLayers() {
    ...
    let stackedLayers = d3
      .stack()
      .offset(d3.stackOffsetSilhouette)
      .keys(fields)(totalFrequencies);
    ...
    fields.forEach((field, i) => {
      ...
      stackedLayers[i].forEach((stackLayer) => {
        boxes[field].push({
          ...
          width: boxWidth,
          height:
            streamSizeScale(stackLayer[1]) - streamSizeScale(stackLayer[0]),
        });
      });
    });
    return boxes;
